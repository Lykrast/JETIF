	...
	public void addRecipes(List<JETIFWrapper> list) {
		...
		try {
			...
			for (Object f : featureFlags) {
				if (!doFluix && (featureKey.invoke(f)).equals("InWorldFluix")) doFluix = true;
				if (!doPurify && (featureKey.invoke(f)).equals("InWorldPurification")) doPurify = true;
			}
		} catch (Exception e) {
		...
		if (doFluix) {
			list.add(new JETIFWrapper(new FluidStack(FluidRegistry.WATER, 1000), null,
					...
					getModdedItem("appliedenergistics2:material", 1, 1), new ItemStack(Items.QUARTZ),
					new ItemStack(Items.REDSTONE)));
		}
		if (doPurify) {
			list.add(new JETIFWrapper(new FluidStack(FluidRegistry.WATER, 1000), null,
					getModdedItem("appliedenergistics2:material", 1, 10),
					getModdedItem("appliedenergistics2:crystal_seed", 1, 0)));
			list.add(new JETIFWrapper(new FluidStack(FluidRegistry.WATER, 1000), null,
					getModdedItem("appliedenergistics2:material", 1, 11),
					getModdedItem("appliedenergistics2:crystal_seed", 1, 600)));
			list.add(new JETIFWrapper(new FluidStack(FluidRegistry.WATER, 1000), null,
					getModdedItem("appliedenergistics2:material", 1, 12),
					getModdedItem("appliedenergistics2:crystal_seed", 1, 1200)));
		}
	}
